...
const analysis = {
  ...
  commercial_products: [{ trade_name: 'VESTAMIN IPD', manufacturer: 'Evonik', mapped_chemical_candidate: 'Isophorone diamine', product_type: 'PURE_CHEMICAL', confidence: 0.75, evidence_quote: 'VESTAMIN IPD', page: 1 }],
  application_categories: [{ name: 'Coating', confidence: 0.97, evidence_quote: 'epoxy coating composition', page: 1 }],
...
const mockServer = http.createServer((request, response) => {
  ...
  request.on('end', () => {
    ...
    response.end(JSON.stringify({ output_text: JSON.stringify(analysis), usage: { input_tokens: 120, output_tokens: 420, total_tokens: 540 } }));
  });
});
...
try {
  ...
  owner = null;
  console.log('EDGE_E2E_PASS analysis=REVIEW_REQUIRED suggestions=5 cross_user=blocked accepted=IPDA account_deleted=true');
} finally {
